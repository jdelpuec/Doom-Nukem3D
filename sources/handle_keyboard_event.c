...
/*   Updated: 2020/01/31 18:53:55 by jdelpuec         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_keyboard_mvt(t_win *w, t_ray *r, t_keyboard *k)
{
	...
	if (r->player.velocity.x > 1.0 || r->player.velocity.x < 1.0 || r->player.velocity.y > 1.0 || r->player.velocity.y < 1.0)
	{
		while (i < r->sectors[r->player.sector].wall_count)
		{
			...
			{
				if (fabsf(check_line_point(wall.p1, wall.p2, new_pos)) < 50)
				{
					printf("%f ; %d ; %d \n", r->player.position.z, r->sectors[r->player.sector].floor_height, r->sectors[r->player.sector].ceil_height);
